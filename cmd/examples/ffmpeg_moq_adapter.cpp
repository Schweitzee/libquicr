...
void
FfmpegToMoQAdapter::OnInit(int stream_index, const uint8_t* init, size_t init_len, bool last_init) const
{
    if (!init || !init_len)
        return;
    ...
    //SPDLOG_DEBUG("trying to get lock for init segment processing - {}\n", stream_index);
    {
        ...
        if (!inserted)
            it->second = tpd; // felülírás, ha már létezett
    }
...
void
FfmpegToMoQAdapter::OnFrag(int stream_index, const uint8_t* moof, size_t moof_len, bool keyframe) const
{
...
void
FfmpegToMoQAdapter::ParseSingleTrackInit(const uint8_t* buffer, size_t buffer_size, CatalogTrackEntry& catalog_data)
{
...
void
FfmpegToMoQAdapter::ProcessSingleTrackMoov(const MP4Atom& moov, CatalogTrackEntry& catalog_data)
{
    ...
    while (offset + 8 <= moov.data.size()) {
        uint32_t box_size = (moov.data[offset] << 24) | (moov.data[offset + 1] << 16) | (moov.data[offset + 2] << 8) |
                            moov.data[offset + 3];
...
void
FfmpegToMoQAdapter::ProcessSingleTrack(const uint8_t* trak_data, uint32_t trak_size, CatalogTrackEntry& catalog_data)
{
    int track_id = -1;
    ...
    while (offset + 8 <= trak_size) {
        uint32_t box_size = (trak_data[offset] << 24) | (trak_data[offset + 1] << 16) | (trak_data[offset + 2] << 8) |
                            trak_data[offset + 3];
                    ...
                    while (minf_offset + 8 <= mdia_offset + mdia_box_size) {
                        ...
                        if (minf_box_type == "stbl") {
                            ...
                            while (stbl_offset + 8 <= minf_offset + minf_box_size) {
                                uint32_t stbl_box_size = (trak_data[stbl_offset] << 24) |
                                                         (trak_data[stbl_offset + 1] << 16) |
                                                         (trak_data[stbl_offset + 2] << 8) | trak_data[stbl_offset + 3];
                                std::string stbl_box_type(reinterpret_cast<const char*>(&trak_data[stbl_offset + 4]),
                                                          4);
