...
static inline bool box_fits(size_t off, size_t size, size_t len) {
    if (size < 8) return false;
    return off + size <= len;
}

// ---- belső segéd: kulcskép-eldöntés a flag-ekből ----
static inline bool sample_is_sync(uint32_t f) {
    ...
    if (!is_non_sync) return true;        // gyakorlatban elég: ha nem non-sync, akkor sync
    return depends_on == 2;               // extra óvatosság, ha jelölve van
}

// ---- belső segéd: egyetlen MOOF bejárása ----
static bool moof_has_keyframe(const uint8_t* buf, size_t moof_off, size_t moof_size, size_t len) {
    ...
    while (off + 8 <= moof_end) {
        ...
        if (box_size < 8 || !box_fits(off, box_size, len)) break;
        const char* typep = reinterpret_cast<const char*>(buf + off + 4);
        uint32_t type = (uint32_t(typep[0]) << 24) | (uint32_t(typep[1]) << 16) |
                        (uint32_t(typep[2]) << 8)  |  uint32_t(typep[3]);
        ...
        if (type == 0x74726166u) {
            ...
            while (tOff + 8 <= traf_end) {
                ...
                if (isz < 8 || !box_fits(tOff, isz, len)) break;
            ...
            while (tOff + 8 <= traf_end) {
                ...
                if (itype == 0x7472756Eu) { // "trun" (FullBox)
                    ...
                    uint32_t trun_flags = (uint32_t(buf[p+1]) << 16) | (uint32_t(buf[p+2]) << 8) | uint32_t(buf[p+3]);
                    p += 4;

                    if (p + 4 > len) break;
                    uint32_t sample_count = be32(buf + p); p += 4;
                    ...
                    if (data_offset_present) { if (p + 4 > len) break; p += 4; }

                    uint32_t first_sample_flags = 0;
                    if (first_sample_flags_present) {
                        if (p + 4 > len) break;
                        first_sample_flags = be32(buf + p); p += 4;
                    }
                    ...
                    for (uint32_t i = 0; i < sample_count; ++i) {
                        ...
                        if (has_sample_duration) { if (rec + 4 > len) break; rec += 4; }
                        if (has_sample_size)     { if (rec + 4 > len) break; rec += 4; }
                        ...
                        if (has_sample_cto) { if (rec + 4 > len) break; rec += 4; }
...
bool CmafChunkHasKeyframe(const uint8_t* buf, size_t len) {
    size_t off = 0;
    while (off + 8 <= len) {
        uint32_t box_size = be32(buf + off);
        if (box_size < 8 || !box_fits(off, box_size, len)) break;
        ...
        if (type == 0x6D6F6F66u) { // "moof"
            if (moof_has_keyframe(buf, off, box_size, len)) return true;
        }
...
int FfmpegCmafSplitter::Run(const std::atomic<bool>& stop) {
        int err;
        ...
        if (err < 0) {
            char e[128]; av_strerror(err, e, sizeof(e));
  ...
  std::map<uint32_t, TrackWriter*> writers;
    ...
    if (ic->nb_streams == 0) { SPDLOG_ERROR("No streams found in input."); return err; }
    ...
    auto pkt_ts_sec = [&](AVPacket* p)->double {
        ...
        if (t == AV_NOPTS_VALUE) return NAN;
        return t * av_q2d(tb);
    };
    ...
    while ((err = av_read_frame(ic, pkt)) >= 0 && !stop.load(std::memory_order_relaxed)) {
        if (cfg_.realtime_pace) {
            if (double ts = pkt_ts_sec(pkt); !std::isnan(ts)) {
                if (std::isnan(start_ts)) start_ts = ts;
        ...
        if (tw->st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            iskeyframe= CmafChunkHasKeyframe(
                tw->mbuf.data.data(), tw->mbuf.data.size());
        } else {
            iskeyframe= true;
        }
