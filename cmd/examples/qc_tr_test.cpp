...
FILE* data_fp = fdopen(3, "wb");  // egyszer a program elején
  ...
  public:
                         ...
                         const quicr::messages::PublishAttributes& publish_attributes) override
    {
        ...
        ResolvePublish(connection_handle,
                       request_id,
                       publish_attributes,
                       { .reason_code = quicr::PublishResponse::ReasonCode::kInternalError });
             ...
             const bool absolute)
{
    ...
    try {
        transcode_client = std::make_shared<transcode::TranscodeClient>(cfg);

    } catch (...) {
        SPDLOG_ERROR("Failed to create TranscodeClient");
        return;
    }

    transcode_client->SetOutputFragmentCallback([](MP4Chunk chunk) {
        SPDLOG_INFO("Transcoded fragment: {} bytes",
                    chunk.whole_chunk.size);
        fwrite(chunk.whole_chunk.data.data(), chunk.whole_chunk.data.size(), 1, data_fp);
        fflush(data_fp);
    });

    transcode_client->SetOutputInitCallback( [] (const uint8_t* data, size_t size) {
        SPDLOG_INFO("Output init segment ready: {} bytes", size);
        fwrite(data, size, 1, data_fp);
        fflush(data_fp);
    });
...
main(int argc, char* argv[])
{
    ...
    if (auto mtc = spdlog::get("MTC")) {
        auto stdout_sink = std::make_shared<spdlog::sinks::stdout_color_sink_mt>();
        mtc->sinks().clear();
        mtc->sinks().push_back(stdout_sink);
    }
