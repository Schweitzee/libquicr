...
static bool caps_starts_with(GstCaps* caps, const char* prefix) {
...
// -------------- BuildPipelines --------------
bool SubscriberGst::BuildPipelines(const char* video_sink, const char* audio_sink) {
    std::lock_guard<std::mutex> lk(mtx_);
    if (!BuildVideoPipeline_(video_sink)) return false;
    if (!BuildAudioPipeline_(audio_sink)) return false;
    return true;
}


bool SubscriberGst::StartPipelines(GstState s) {
    std::lock_guard<std::mutex> lk(mtx_);
    bool ok = true;
    if (vpipe_) ok &= (gst_element_set_state(vpipe_, s) != GST_STATE_CHANGE_FAILURE);
    if (apipe_) ok &= (gst_element_set_state(apipe_, s) != GST_STATE_CHANGE_FAILURE);
    return ok;
}
bool SubscriberGst::SetVideoState(GstState s) {
    std::lock_guard<std::mutex> lk(mtx_);
    return vpipe_ && (gst_element_set_state(vpipe_, s) != GST_STATE_CHANGE_FAILURE);
}
bool SubscriberGst::SetAudioState(GstState s) {
    std::lock_guard<std::mutex> lk(mtx_);
...
// -------------- Video pipeline --------------
bool SubscriberGst::BuildVideoPipeline_(const char* video_sink) {
  vpipe_ = gst_pipeline_new("moq-vpipe");
  if (!vpipe_) { g_printerr("Failed to create vpipe\n"); return false; }
  ...
  // Static links
  if (!gst_element_link(video_src_, vdemux_)) { g_printerr("link video_src->vdemux failed\n"); return false; }
  if (!gst_element_link(vqueue_, vdec_))      { g_printerr("link vqueue->vdec failed\n"); return false; }
  if (!gst_element_link_many(vconv_, vscale_, vsink_, NULL)) { g_printerr("link vconv->vscale->vsink failed\n"); return false; }

  // Demux pad-added: link the first pad to vqueue sink (no caps filtering)
  g_signal_connect(vdemux_, "pad-added",
  ...
  g_signal_connect(vdec_, "pad-added",
...
// -------------- Audio pipeline --------------
bool SubscriberGst::BuildAudioPipeline_(const char* audio_sink) {
  apipe_ = gst_pipeline_new("moq-apipe");
  if (!apipe_) { g_printerr("Failed to create apipe\n"); return false; }
  ...
  if (!gst_element_link(audio_src_, ademux_)) { g_printerr("link audio_src->ademux failed\n"); return false; }
  if (!gst_element_link(aqueue_, adec_))      { g_printerr("link aqueue->adec failed\n"); return false; }
  if (!gst_element_link_many(aconv_, ares_, asink_, NULL)) { g_printerr("link aconv->ares->asink failed\n"); return false; }

  // Demux pad-added: link first pad to aqueue
  g_signal_connect(ademux_, "pad-added",
    G_CALLBACK(+[](GstElement* demux, GstPad* new_pad, gpointer user_data){
  ...
  g_signal_connect(adec_, "pad-added",
    G_CALLBACK(+[](GstElement* decodebin, GstPad* new_pad, gpointer user_data){
...
// -------------- Register init --------------
bool SubscriberGst::RegisterTrackInit(const std::string& track_name, bool is_video,
                                      const uint8_t* init, size_t init_len) {
    std::lock_guard<std::mutex> lk(mtx_);
    if (!init || init_len == 0) return false;
...
bool SubscriberGst::SelectVideo(const std::string& track_name) {
    std::lock_guard<std::mutex> lk(mtx_);
    auto it = init_by_name_.find(track_name);
    if (it == init_by_name_.end() || !it->second.is_video || !video_src_) return false;
...
bool SubscriberGst::SelectAudio(const std::string& track_name) {
    std::lock_guard<std::mutex> lk(mtx_);
    auto it = init_by_name_.find(track_name);
    if (it == init_by_name_.end() || it->second.is_video || !audio_src_) return false;
...
bool SubscriberGst::VideoPushFragment(const uint8_t* data, size_t len,
                                      bool is_rap) {
  std::lock_guard<std::mutex> lk(mtx_);
  if (!video_src_ || !data || len == 0) return false;
  ...
  if (dis) GST_BUFFER_FLAG_SET(b, GST_BUFFER_FLAG_DISCONT);
  need_discont_video_ = false;
  if (!is_rap) GST_BUFFER_FLAG_SET(b, GST_BUFFER_FLAG_DELTA_UNIT);

  return gst_app_src_push_buffer(GST_APP_SRC(video_src_), b) == GST_FLOW_OK;
}

bool SubscriberGst::AudioPushFragment(const uint8_t* data, size_t len,
                                      bool is_rap) {
  std::lock_guard<std::mutex> lk(mtx_);
  if (!audio_src_ || !data || len == 0) return false;
...
bool SubscriberGst::VideoPushInit(const uint8_t* data, size_t len) {
  std::lock_guard<std::mutex> lk(mtx_);
  if (!video_src_ || !data || len == 0) return false;
  GstBuffer* b = gst_buffer_new_allocate(NULL, len, NULL);
  GstMapInfo mi;
...
bool SubscriberGst::AudioPushInit(const uint8_t* data, size_t len) {
  std::lock_guard<std::mutex> lk(mtx_);
  if (!audio_src_ || !data || len == 0) return false;
  GstBuffer* b = gst_buffer_new_allocate(NULL, len, NULL);
  GstMapInfo mi;
...
// =============== Accessors ===============
std::string SubscriberGst::CurrentVideo() const {
  std::lock_guard<std::mutex> lk(const_cast<std::mutex&>(mtx_));
  return current_video_;
}
std::string SubscriberGst::CurrentAudio() const {
  std::lock_guard<std::mutex> lk(const_cast<std::mutex&>(mtx_));
