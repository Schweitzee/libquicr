 ...
 */
int WritePacket(void* opaque, const uint8_t* buf, int buf_size)
{
  ...
  private:
    ...
    bool ProcessFragment(const uint8_t* data, size_t size)
    {
        ...
        if (!decoder_ctx_) {
            ...
            if (config_.debug) {
                SPDLOG_INFO("Decoder initialized: {}x{} pix_fmt: {} codec: {}, framerate: {}, timebase: {}",
                            decoder_ctx_->width,
                            decoder_ctx_->height,
                            av_get_pix_fmt_name(decoder_ctx_->pix_fmt),
                            avcodec_get_name(codecpar->codec_id),
                            frag_fmt_ctx->streams[video_idx]->r_frame_rate.num,
                            frag_fmt_ctx->streams[video_idx]->time_base.num);
            }
        }
